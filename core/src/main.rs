...
impl ExGSeEngine {
    ...
    /// Generate AI-powered script from session (simplified for now)
    async fn generate_ai_script(&self, timeline: &[SimpleTimelineEntry]) -> Result<String> {
        // For now, generate a basic script with project context
        // AI integration will be added in v4.0.4 after fixing compilation issues
        let project_context = extract_project_context();
        ...
        // Project info
        if let Some(name) = &project_context.name {
            md.push_str(&format!("**Project:** {}\n", name));
        }
        if !project_context.tech_stack.is_empty() {
            md.push_str(&format!("**Tech Stack:** {}\n", project_context.tech_stack.join(", ")));
        }
        md.push_str("\n");

        md.push_str("## Act 1 - Development Session\n\n");
        md.push_str("### Scene 1 - Session Overview\n\n");
        md.push_str(&format!("The developer worked for {} seconds", (Utc::now() - self.start_time).num_seconds()));

        if !timeline.is_empty() {
            md.push_str(" with the following key events:\n\n");
            for (i, entry) in timeline.iter().enumerate() {
                md.push_str(&format!("#### {} - {}\n\n", i + 1, entry.timestamp));
                md.push_str(&format!("**{}:** {}\n\n", entry.event_type, entry.description));
            }
        } else {
            md.push_str(".\n\n");
        }

        md.push_str("---\n*Generated by EX-G-SE v4.0.3*\n");
        md.push_str("\n\n> **Note:** Configure AI with `exg config` to get enhanced scripts with thought process analysis and technical insights.\n");

        Ok(md)
    }

    /// Generate script text without AI (basic version)
    fn generate_basic_script_text(&self, timeline: &[SimpleTimelineEntry]) -> String {
        let project_context = extract_project_context();
        ...
        md.push_str("---\n*Generated by EX-G-SE v4.0.3*\n");
        md.push_str("\n\n> **AI Enhancement:** Configure AI with `exg config` to get enhanced scripts with thought process analysis and technical insights (coming in v4.0.4).\n");
    ...
    async fn run(&mut self) -> Result<()> {
        eprintln!("EX-G-SE Core Engine v0.5.4 - Ghost Mode");
        eprintln!("========================================");
